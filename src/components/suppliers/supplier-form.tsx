...
export function SupplierForm({ id, defaultValues }: SupplierFormProps) {
  ...
  const { register, control, handleSubmit, formState: { errors, isSubmitting } } = useForm<SupplierFormValues>({
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 max-w-lg">
      ...
      {isEditing && (
        <div className="flex items-center gap-3">
          ...
          />
          <Label htmlFor="isActive" className="cursor-pointer">Fornecedor ativo</Label>
        </div>
      )}
